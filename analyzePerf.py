...
def handle_line(line, s):
    ...
    if line.startswith("FFBuild-"):
        ...
        s.currentTestName = "Final_JPEG_Time_all_inv"
        s.currentValueName = float(line.split(" all_inv=")[1].split(" ")[0].replace(',', ''))
        addTestValue(s)
        ...
        s.currentTestName = "Final_PNG_Time_all_inv"
        s.currentValueName = float(line.split(" all_inv=")[1].split(" ")[0].replace(',', ''))
        addTestValue(s)
...
def print_final_results2(s, opFile):
    ...
    rows = 0
    for test, buildObj in s.timings.items():
        curr = 1 + len(s.timings[test].items())
        if rows < curr:
            anykey = test
            rows = curr
...
def main():
    ...
    directory = os.path.dirname(argv2)
    if not os.path.exists(directory):
        os.makedirs(directory)
    print_final_results(s, argv2)
    print_final_results2(s, argv2)
